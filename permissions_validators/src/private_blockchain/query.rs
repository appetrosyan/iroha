...
impl IsAllowed for OnlyAccountsDomain {
    ...
    fn check(
        ...
        use QueryBox::*;
        todo!()
        // if let Some(authority_domain) = authority.domain_id() {
        //     match query {
        ...
        //             if account_id.domain_id == *authority_domain {
        //                 Allow
        //             } else {
        //                 Deny(format!(
        ...
        //             if asset_id.account_id.domain_id == *authority_domain {
        //                 Allow
        //             } else {
        //                 Deny(format!(
        ...
        //             if *account_id.domain_id().expect("Valid") == *authority_domain {
        //                 Allow
        //             } else {
        //                 Deny(format!(
        //                     "Cannot access account {} as it is in a different domain.",
        ...
        //             if asset_definition_id.domain_id == *authority_domain {
        //                 Allow
        //             } else {
        //                 Deny(format!(
...
impl IsAllowed for OnlyAccountsData {
    ...
    fn check(
        ...
        match query {
            ...
            FindAccountIdByAlias(query) => {
                let account_id = try_evaluate_or_deny!(query.alias, wsv);
                if let Some(ref authority_alias) = authority.alias {
                    if account_id == *authority_alias {
                        Allow
                    } else {
                        Deny(format!(
